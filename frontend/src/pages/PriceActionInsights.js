  ...
  return (
    ...
    >
      ...
      <div className="p-4 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className={`text-xs px-2 py-0.5 rounded-full font-bold ${b.badge}`}>
              <BiasIcon className="inline w-3 h-3 mr-1" />
              {signal.bias === 'bullish' ? 'BUY' : signal.bias === 'bearish' ? 'SELL' : 'NEUTRAL'}
            </span>
          </div>
        ...
        <div className="shrink-0 text-right">
          <p className="text-lg font-mono font-bold text-white">{signal.current_price}</p>
          <p className="text-xs text-gray-500 mt-0.5">{Math.round(signal.setup_quality)}% quality</p>
      ...
      <div className="h-1 bg-white/5">
        <motion.div
          initial={{ width: 0 }}
          animate={{ width: `${signal.setup_quality}%` }}
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className={`font-semibold ${signal.location && signal.location !== 'Range' ? 'text-amber-400' : 'text-gray-400'}`}>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between p-2 rounded-lg bg-white/5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-300">{signal.bias === 'bullish' ? 'Buy Entry' : signal.bias === 'bearish' ? 'Sell Entry' : 'Entry'}</span>
                      </div>
...
function PriceActionInsights() {
  ...
  const neutralCount = allSignals.filter(s => s.bias === 'neutral').length;
  const highProbCount = allSignals.filter(s => s.score >= 70).length;
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          {lastFetched && (
            <span className="ml-auto text-xs text-gray-500">
              Last scan: {lastFetched.toLocaleTimeString()} · {allSignals.length} pairs{minScore > 0 ? ` · ${signals.length} shown` : ''}
            </span>
          )}
        </div>
      ...
      <div className="space-y-3">
        ...
        {!loading && allSignals.length === 0 && (
          ...
          >
            ...
            <p className="text-white font-semibold mb-1">No signals received</p>
            <p className="text-gray-400 text-sm">
              The server may still be deploying. Try clicking Refresh in a minute.
            </p>
          </motion.div>
        )}

        {!loading && allSignals.length > 0 && signals.length === 0 && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="glass-card border border-white/5 p-10 text-center"
          >
            ...
            <p className="text-gray-400 text-sm">
              Lower the Min Score or drag slider to 0 to show all pairs.
            </p>
          </motion.div>
        )}
